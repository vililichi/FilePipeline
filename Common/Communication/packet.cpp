...
Packet::Packet(const size_t beginCapacity)
    : m_capacity(beginCapacity), m_size(0), m_cursor(0)
{
    m_data = new uint8_t[m_capacity]();
}

Packet::Packet(const Packet& base)
    : m_capacity(base.m_capacity), m_size(base.m_size), m_cursor(base.m_cursor)
{
    m_data = new uint8_t[m_capacity];
...
void Packet::setCapacity(const size_t newCapacity_)
{
    ...
    if ((new_data = new uint8_t[newCapacity_]) == nullptr)
    {
        throw "echec de l'assignation";
    }

    size_t limit = m_size;
    if (newCapacity_ < limit)
        limit = newCapacity_;
...
{
    if (position_ > m_size)
        throw "deplacement à l'exterieur des donnees";
    m_cursor = position_;
}

void Packet::add(const uint8_t* const newData_, const size_t dataSize_)
{
...
{
    if (((long)m_size - (long)nbr_) < 0)
        throw "nbr insufisant de data dans le packet";
    m_size -= nbr_;
    if (m_cursor > m_size)
        m_cursor = m_size;
}
...
void Packet::read(uint8_t* const data_, const size_t readSize_) const
{
    if ((m_cursor + readSize_) > m_size)
