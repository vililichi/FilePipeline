...
public:
	...
	*/
	Packet(const size_t beginCapacity_ = INIT_PACKET_SIZE); //constructeur avec capacité de départ
	...
	// Ajoute les données à partir de la position du curseur et déplace le curseur à la fin des données
	// [in] newData_  : Pointeur vers le début des données à ajouter
	// [in] dataSize_ : Taille des données à ajouter
