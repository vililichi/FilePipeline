...
void AES::cryptage(Packet& cible_, uint8_t* cle_)
{
    ...
    for (int i = 0; i < cycle; i++)
    {
        futures.emplace_back(std::async(
            std::launch::async,
            [](uint8_t* b16, uint8_t* cle) { AES::cryptage(b16, cle); },
...
void AES::decryptage(Packet& cible_, uint8_t* cle_)
{
    ...
    for (int i = 0; i < cycle; i++)
    {
        futures.emplace_back(std::async(
            std::launch::async,
            [](uint8_t* b16, uint8_t* cle) { AES::decryptage(b16, cle); },
            cible_.data() + (16 * i), cle_));
    }
    for (auto& future : futures)
        future.wait();
    ...
    if (addTaille > 16 || addTaille < 1)
        throw "dernier bytes impossible, il y a eu un probleme le packet n'est pas "
              "valide";
