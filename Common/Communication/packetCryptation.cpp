...
void AES::cryptage(Packet& cible_, char* cle_)
{
	...
	{
		futures.emplace_back(std::async(std::launch::async, [](char* b16, char* cle) {AES::cryptage(b16, cle); }, cible_.data() + (16 * i), cle_));
	}
...
void AES::decryptage(Packet& cible_, char* cle_)
{
	...
	for (int i = 0; i < cycle; i++)
	{
		futures.emplace_back(std::async(std::launch::async, [](char* b16, char* cle) {AES::decryptage(b16, cle); }, cible_.data() + (16 * i), cle_));
	}
	for (auto& future : futures)future.wait();
