              ...
              const std::string& folder, bool ui)
{
    ...
    while (tailleActu < tailleFichier)
    {
        ...
        if (ui)
        {
            ...
            if (trigger == 0)
            {
                std::cout << '\r' << tailleActu << " octets\t\t| " << tailleFichier
                          << " octets\t" << tachy.speed() << "Ko/s";
                trigger = 1024;
            }
            else
                trigger--;
    ...
    if (ui)
    {
        std::cout << '\r' << tailleActu << " octets\t\t| " << tailleFichier << " octets\t"
                  << tachy.speed() << "Ko/s";
        tachy.stop();
        chronoTotal.stop();
        std::cout << std::endl
                  << "vitesse moyenne: " << tachy.avgSpeed() << "ko/s" << std::endl;
